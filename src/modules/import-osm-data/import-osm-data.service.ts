...
export class ImportOsmDataService {
  public DATA_IMPORT: string;
  public osmDataPath: string;
  public linkSplit: string;
  ...
  constructor(
    ...
    this.DATA_IMPORT = getRequiredEnvStr(this.configService, 'DATA_IMPORT');
    this.osmDataPath = getRequiredEnvStr(this.configService, 'OSM_DATA_PATH');
    this.linkSplit = getRequiredEnvStr(this.configService, 'LINK_SPLIT');
    this.splitError = getRequiredEnvStr(this.configService, 'SPLIT_ERROR');

    const searchExpansionStr = getRequiredEnvStr(
      this.configService,
      'NODE_SEARCH_EXPANSION',
    );
    this.searchExpansion = parseFloat(searchExpansionStr);
    if (isNaN(this.searchExpansion)) {
      throw new Error('NODE_SEARCH_EXPANSION must be a valid number.');
    }
  }

  async processNodeLinkData() {
    const startDataProcessing = this.processStartTime('processNodeLinkData');
    const filePathList = await this.fetchHighwayData();
    await this.generateNodesFromLinkFiles(filePathList);

    this.processEndTime('processNodeLinkData', startDataProcessing);
  }

  async fetchHighwayData(): Promise<string[]> {
    // 3600307756 : South_Korea
    // 3600536780 : Singapore
    // 3600307763 :	United_Arab_Emirates
    // 3600307584 : Saudi_Arabia
    // 3602297418 : Seoul
    const filePathList: string[] = [];
    const osmAreaId = 3600307756;
    // const osmAreaId = 3600536780;
    // const osmAreaId = 3600307763;
    // const osmAreaId = 3600307584;
    ...
    const getCountryName = async (): Promise<string> => {
      try {
        ...
        return (
          ...
          matched?.tags?.name ||
          `unknown-country-${osmAreaId - 3600000000}`
        );
      } catch (err) {
        if (err instanceof Error) {
          console.warn(`Failed to fetch country name: ${err.message}`);
        } else {
          console.warn('Failed to fetch country name: unknown error');
        }
    ...
    try {
      ...
      try {
        ...
        if (areaInfoList.length < 2) {
          this.logToFile(
            `admin_level=4 결과 부족 (${areaInfoList.length}개), admin_level=6 재시도`,
            this.DATA_IMPORT,
          );
    ...
    for (const { areaId, admin_ko, admin_en, country } of areaInfoList) {
      ...
      while (!success && attempts < maxRetries) {
        try {
          ...
          success = true;
          const completeMsg = `${admin_ko} (${areaId}) 완료`;
          console.log(`completeMsg`);
          this.logToFile(completeMsg, this.DATA_IMPORT);
        } catch (err) {
          attempts++;
          ...
          this.logToFile(attemptFaileMsg, this.DATA_IMPORT);
        }
      }
    ...
    for (const { areaId, admin_ko, admin_en, country } of failedAreas) {
      try {
        ...
        if (data?.elements?.length) {
          ...
          filePathList.push(savedPath);
          geoJsonData.features.length = 0;
          const retryMsg = `재시도 성공: ${admin_ko} - ${admin_en} (${areaId})`;
          console.log(`retryMsg`);
          this.logToFile(retryMsg, this.DATA_IMPORT);
        }
      } catch (err) {
        const failMsg =
          err instanceof Error
            ? `최종 실패: ${admin_en} (${areaId}) - ${err.message}`
            : `최종 실패: ${admin_en} (${areaId}) - unknown error`;
        console.log(`failMsg`);
        this.logToFile(failMsg, this.DATA_IMPORT);
      }
    }
  ...
  async generateNodesFromLinkFiles(filePathList: string[]): Promise<void> {
    ...
    console.log('선택된 국가의 Node 파일 생성 완료');
  }
  ...
  async generateNodeFromLinkFileInDB(
    ...
    try {
      ...
      console.log(`Node 생성 완료: ${outputPath}`);
    } catch (err) {
      if (err instanceof Error) {
        console.warn(`Node 생성 실패 (${linkFilePath}): ${err.message}`);
      } else {
        console.warn(`Node 생성 실패 (${linkFilePath}): unknown error`);
      }
    } finally {
      // 4. 테이블 삭제
      await this.dataSource.query(`DROP TABLE IF EXISTS ${tempTable}`);
  ...
  private saveLinkJSONToFile(
    ...
    try {
      ...
      this.logToFile(jsonSavedMsg, this.DATA_IMPORT);
    } catch (error) {
      const jsonErrorMsg =
      ...
      this.logToFile(jsonErrorMsg, this.DATA_IMPORT);
      throw error;
    }
  }
  ...
  private processStartTime(funcName: string): number {
    ...
    this.logToFile(processStartMessage, this.DATA_IMPORT);
  ...
  private processEndTime(funcName: string, startTime: number): void {
    ...
    this.logToFile(processEndMessage, this.DATA_IMPORT);

    const endTime = Date.now();
    const durationMessage = `${funcName} took ${endTime - startTime}ms`;
    console.log(durationMessage);
    this.logToFile(durationMessage, this.DATA_IMPORT);
  }
